...
{#if open}
  <div class="mc-modal-backdrop" role="presentation" onclick={onClose}>
    <div
      ...
      class="mc-modal mc-editor-modal"
    >
      <div class="mc-modal__header">
        <h2 class="mc-modal__title">Editar imagen</h2>
        <button type="button" onclick={onClose} aria-label="Cerrar" class="mc-modal__close">✕</button>
      </div>

      <div class="mc-modal__body mc-editor-modal__body">
        <div class="mc-editor-modal__left">
          <div bind:this={containerEl} class="mc-editor-modal__preview">
            {#if previewUrl}
              <img
              src={previewUrl}
              alt="Editor preview"
            ...
            />
            {/if}
            <div style={overlayStyle} class="mc-editor-modal__crop-overlay">
          ...
          <div class="mc-editor__controls">
            ...
            {#each aspectPresets as preset (preset)}
              <label>
                <input type="radio" name="mc-aspect" checked={aspect === preset} onchange={() => selectPreset(preset)} />
                {preset}
              </label>
            {/each}
            <label>
        ...
        <div class="mc-editor-modal__right">
          <h3>Metadatos</h3>
          <label class="mc-field">
            Nombre del archivo <span class="mc-editor-modal__badge">editable</span>
            <input type="text" class="mc-input" bind:value={metaFileName} />
          </label>
          <label class="mc-field">
            Texto alternativo (ALT)
            <input type="text" class="mc-input" bind:value={metaAlt} />
          </label>
          <label class="mc-field">
            Título
            <input type="text" class="mc-input" bind:value={metaTitle} />
          </label>
          <label class="mc-field">
            Descripción
            <textarea class="mc-input" bind:value={metaDescription} rows="3"></textarea>
          </label>
          <button type="button" onclick={handleSaveMetadata} class="mc-button">Guardar metadatos</button>
        </div>
      </div>

      <div class="mc-modal__footer">
        <button type="button" onclick={handleCancel} class="mc-button">Cancelar</button>
        <button type="button" onclick={handleOverwrite} disabled={!sourceKey || !provider || isBusy} class="mc-button">
          Sobreescribir
        </button>
        <button type="button" onclick={handleSaveAsNew} disabled={!provider || isBusy} class="mc-button">
          Guardar como nuevo
        </button>
      </div>
    </div>
