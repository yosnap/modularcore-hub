  ...
  template: `
    <div>
      <div *ngIf="token" role="status">
        ...
        <p *ngIf="authKit.state().verifyEmail.status === 'success'">Your email is verified.</p>
      ...
      <form novalidate (submit)="handleResendSubmit($event)">
        ...
        <p *ngIf="authKit.state().resendVerification.status === 'success'">Verification email sent.</p>
      </form>
    </div>
  `,
})
