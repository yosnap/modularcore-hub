...
<form onsubmit={handleSubmit} novalidate>
  <div>
    ...
    <button type="button" onclick={() => (showCurrent = !showCurrent)}>{showCurrent ? 'Hide' : 'Show'}</button>
  ...
  <div>
    ...
    <button type="button" onclick={() => (showNew = !showNew)}>{showNew ? 'Hide' : 'Show'}</button>
  ...
  {#if flow.status === 'success'}<p>Password updated.</p>{/if}
</form>
