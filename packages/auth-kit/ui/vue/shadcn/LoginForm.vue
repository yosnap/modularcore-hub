...
<template>
  <form novalidate class="flex flex-col gap-4" @submit.prevent="handleSubmit">
    ...
    <p v-if="authKit.state.value.login.status === 'success'" class="text-sm text-green-600">
      Sesión iniciada.
    </p>

    <p v-if="onNavigateToRegister" class="text-center text-sm text-muted-foreground">
      ¿Aún no tienes una cuenta?
      <button type="button" :class="linkClass" @click="onNavigateToRegister">Registrarse</button>
    </p>
  </form>
</template>
