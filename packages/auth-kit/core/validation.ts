...
export function buildPasswordSchema(policy: PasswordPolicy = {}) {
  ...
  let schema = z
    .string()
    .min(p.minLength, `La contraseña debe tener al menos ${p.minLength} caracteres`);
  if (p.requireUppercase)
    schema = schema.regex(/[A-Z]/, 'La contraseña debe contener una letra mayúscula');
  if (p.requireLowercase)
    schema = schema.regex(/[a-z]/, 'La contraseña debe contener una letra minúscula');
  if (p.requireNumber) schema = schema.regex(/\d/, 'La contraseña debe contener un número');
  if (p.requireSpecial)
    schema = schema.regex(/[^A-Za-z0-9]/, 'La contraseña debe contener un carácter especial');
  return schema;
}
...
export function buildRegisterSchema(fields: ResolvedFieldConfig, options: BuildSchemaOptions = {}) {
  ...
  if (fields.firstName.enabled) {
    shape.firstName = fields.firstName.required
      ? z.string().min(1, 'Obligatorio')
      : z.string().optional();
  }
  if (fields.lastName.enabled) {
    shape.lastName = fields.lastName.required
      ? z.string().min(1, 'Obligatorio')
      : z.string().optional();
  }
  ...
  if (fields.profileType.enabled) {
    const values = fields.profileType.options.map((option) => option.value) as [
      string,
      ...string[],
    ];
