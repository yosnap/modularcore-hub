<script lang="ts">
  ...
  // `maxWidthClass` (safe/style.ts) stays the single source of truth for which enum value is
  // "valid" — it returns package-scoped tokens (`modals-max-w-*`), not real utility classes, so
  // every UI variant maps those tokens onto its own visual vocabulary here (same pattern as the
  // `tailwind` variant's OverlayBody). This map is a 1:1 translation, never a re-derivation of
  // the enum itself.
  const SHADCN_MAX_WIDTH: Record<string, string> = {
  ...
  const maxWidth = $derived(SHADCN_MAX_WIDTH[maxWidthClass(config.maxWidth)] ?? '');
</script>

<div
  class="modals-body relative w-full {maxWidth} rounded-lg p-4 {!bgColor ? 'bg-background' : ''} {!textColor
    ? 'text-foreground'
    : ''} {extraClass}"
...
>
  ...
  {/if}
  {#if config.title}
    <h2 class="modals-title mb-2 pr-6 text-lg font-semibold leading-tight text-foreground">{config.title}</h2>
  {/if}
  {#if message.html}
    <div class="modals-message mb-4 text-sm leading-relaxed text-muted-foreground">{@html message.html}</div>
  {:else}
    <p class="modals-message mb-4 text-sm leading-relaxed text-muted-foreground">{message.text}</p>
  {/if}
  {#if config.primaryButton || config.secondaryButton}
    <div class="modals-actions flex flex-wrap items-center gap-2">
      {#if config.primaryButton}
        {#if primaryHref}
          <a
            class="modals-button modals-button--primary rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:opacity-90"
            href={primaryHref}
      ...
      {#if config.secondaryButton}
        {#if secondaryHref}
          <a
            class="modals-button modals-button--secondary rounded-md border border-input px-4 py-2 text-sm font-medium text-foreground hover:bg-accent"
            href={secondaryHref}
            rel="noopener noreferrer"
            target="_blank"
