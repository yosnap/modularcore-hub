...
La lista de frameworks **no está en el código de la CLI**: sale del catálogo que publica el registry (`frameworks.json`), que reúne los seis de casa con los que aporte cualquier componente en su `frameworkDefs`. Por eso `init` pregunta la URL del registry antes que nada. Si el registry no responde, avisa y sigue con los frameworks que la CLI ya conoce, para no depender de la red.
...
- [`add`](/1-0-0/referencia/herramientas/cli/add/) — usa `modularcore.json` para saber dónde escribir los componentes
- [`update`](/1-0-0/referencia/herramientas/cli/update/) — reinyecta componentes ya instalados
