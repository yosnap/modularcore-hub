...
3. Para cada componente resuelto, comprueba compatibilidad: que el componente soporte el `framework` declarado en `modularcore.json` (o sea `agnostic`), y que las `peerDependencies` relevantes (React, Svelte, Vue, `@angular/core`, según el framework del proyecto) estén declaradas en el `package.json` del proyecto con un rango semver compatible. En un proyecto `vanilla` no se exige ningún peer de framework: el binding sin framework no carga ninguno.

Del componente se escriben sólo los ficheros que sirven al framework del proyecto, no el descriptor entero. El framework de cada fichero se deduce de su ruta —`adapters/<framework>/`, `ui/<framework>/`, `snippets/astro/` (vanilla) y `snippets/laravel/` (blade)—; todo lo demás, como `core/` o las hojas de estilo sueltas de `ui/`, es compartido y llega siempre. Un fichero cuya ruta no siga esa convención se trata como compartido: ante la duda, sobra un fichero antes que falte uno. Un proyecto Blade recibe además los ficheros `vanilla`, porque sus plantillas no tienen runtime propio en el navegador y montan ese mismo código.
...
- [Visión general del CLI](/1-0-0/referencia/herramientas/cli/)
- [`init`](/1-0-0/referencia/herramientas/cli/init/) — requisito previo, genera `modularcore.json`
- [`diff`](/1-0-0/referencia/herramientas/cli/diff/) — compara un componente ya instalado con la versión del registry
- [`update`](/1-0-0/referencia/herramientas/cli/update/) — reinyecta un componente ya instalado
