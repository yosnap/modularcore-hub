...
5. Provider `s3-compatible`: presigned PUT/POST (config endpoint/bucket/region); probar contra MinIO local.
6. Provider `cloudinary`: upload (unsigned preset o signed); probar contra cuenta de test.
...
9. Tests: crop/compress deterministas (jsdom/canvas mock), upload contra provider mock + smoke real MinIO.
